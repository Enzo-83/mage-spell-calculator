...
| `spellCompendium.js` | index.html, wizard.html | Firestore-backed global spell compendium, **one shared React implementation** mounted on both pages (Phase 10). Exposes `window.compendium` (service: `useSpells`/`useAuth` hooks, CRUD, suggestions, admin, CSV/JSON import-export, and pure helpers `csvToSpells`/`validateSpell`/`sanitiseSpell` for the harness) and `window.CompendiumPanel` (no-JSX React component; `variant: 'classic'\|'wizard'` is the only layout fork — Classic = arcanum-grouped grid, Wizard = flat rows; admin tooling is Classic-only). Role-based editing (admin/editor/sub-editor/suggester). |
